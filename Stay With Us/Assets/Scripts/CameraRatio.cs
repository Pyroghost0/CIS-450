...
public class CameraRatio : MonoBehaviour
{
    ...
    public void AddShopCanvas(UnityEngine.UI.CanvasScaler canvasScaler)
    {
        ...
        if (scaleheight > 1.0f)
        {
            canvasScaler.matchWidthOrHeight = (Mathf.Log(450f * Screen.width / Screen.height) - (5f * Mathf.Log(2f)) - (2f * Mathf.Log(5f))) / (Mathf.Log(Screen.width) - (2 * Mathf.Log(2f)) - Mathf.Log(Screen.height / 3f));
        }
    }
}
